export const mergeSortObj = {
  merge(fstHalf, sndHalf) {
    ...
    while (fstHalf.length && sndHalf.length) {
      if (fstHalf[0] < sndHalf[0]) result.push(fstHalf.shift());
      else if (fstHalf[0] > sndHalf[0]) result.push(sndHalf.shift());
      else result.push(fstHalf.shift(), sndHalf.shift());
    }
  ...
  name: 'Merge Sort',
  placeholder: 'Numbers',
  raw: `const merge = (fstHalf, sndHalf) => {
  const result = [];
  while (fstHalf.length && sndHalf.length) {
