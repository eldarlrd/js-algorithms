export const checkCashRegisterObj = {
  myFunc(inputArr) {
    ...
    if (change < 0) return `The customer owes you $${-change.toFixed(2)}`;
    ...
    if (drawer.total === change) return `status: CLOSED => change: ${cid.join(' | ')}`;
  ...
  raw: `const checkCashRegister = (price, cash, cid) => {
  // Declaring the Default Currencies
  const currArr = [
    {name: 'ONE HUNDRED', val: 100},
    {name: 'TWENTY', val: 20},
